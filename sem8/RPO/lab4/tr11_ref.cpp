...
public:
    ...
    static void processTransaction(BankAccount* account, const std::string& transaction) {
        if (account != nullptr) {
...
int main() {
    ...
    for (const auto& transaction : transactions) {
        ...
        for (auto acc : accounts) {
            if (acc->id() == transaction.first) {
                account = acc;
                break;
            }
        }
        Bank::processTransaction(account, transaction.second);
    }
