<body class="bg-gray-200">
    ...
    if (isset($model['posts']) && count($model['posts']) > 0) {
        foreach ($model['posts'] as $post) {
            ...
            echo "<img src='/lab3/public/assets/img/" . $post['image'] . "' alt='" . $post['image'] . "' class='mb-4 w-1/5'>";
    ...
    <div class="<?= $model['total_pages'] == 0 ? 'hidden' : '' ?>">
        <div class="flex justify-center items-center">
            <a href="/lab3/Blog/index/?page=<?= $model['current_page'] - 1 == 0 ? 1 : $model['current_page'] - 1 ?>"
            ...
            <div class="flex items-center justify-center h-10 mx-2">
                ...
                for ($i = 1; $i <= $model['total_pages']; $i++) {
                    if (!($i == $model['current_page'])) {
                        echo '<a href="/lab3/Blog/index/?page=' . $i . '" class="px-3 py-1 rounded-md text-gray-700 border border-gray-300 hover:bg-gray-200 hover:text-gray-800 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2">' . $i . '</a>';
                    } else {
                        echo '<a href="/lab3/Blog/index/?page=' . $i . '" class="px-3 py-1 rounded-md text-gray-700 border border-gray-300 hover:bg-gray-200 hover:text-gray-800 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2">' . $i . '</a>';
                    }
                }
            ...
            <a href="/lab3/Blog/index/?page=<?= $model['current_page'] + 1 > $model['total_pages'] ? $model['total_pages'] : $model['current_page'] + 1 ?>"
